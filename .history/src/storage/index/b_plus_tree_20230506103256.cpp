...
void BPLUSTREE_TYPE::InsertInParent(BPlusTreePage *old_page, const KeyType &key, BPlusTreePage *new_page, Transaction *transaction = nullptr) {
  ...
  // 插入后大于最大值需要分裂
  auto *mem = new char[INTERNAL_PAGE_HEADER_SIZE + sizeof(MappingType) * (parent_inter_page->GetSize() + 1)];
  InternalPage *merge_page = reinterpret_cast<IndexIterator *>(mem);
  std::memcpy(mem, parent_page->GetData(), INTERNAL_PAGE_HEADER_SIZE + sizeof(MappingType) * (parent_inter_page->GetSize()));
  merge_page->Insert(key, new_page->GetPageId(), comparator_);
