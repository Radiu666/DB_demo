...
auto NestIndexJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  ...
  while (child_executor_->Next(&left_tuple, &emit_rid)) {
    ...
    if (!rids.empty()) {
      ...
      for (uint64_t idx = 0; idx < child_executor_->GetOutputSchema().GetColumnCount(); idx++) {
        vals.emplace_back(left_tuple.GetValue(&child_executor_->GetOutputSchema(), idx));
      }
      for (uint64_t idx = 0; idx < plan_->InnerTableSchema().GetColumnCount(); idx++) {
